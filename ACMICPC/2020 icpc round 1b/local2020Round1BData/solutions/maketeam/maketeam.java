// Arup Guha
// 8/28/2020
// Solution to 2020 UCF Locals Round 1B Problem: Make the Team

import java.util.*;

public class maketeam {
	
	final public static int MAXT = 1000;
	...
	public static void main(String[] args) {
		...
		// lecs[i] will store which lecs are given at time i+1.
		ArrayList<Integer>[] lecs = new ArrayList[MAXT];
		for (int i=0; i<MAXT; i++) lecs[i] = new ArrayList<Integer>();
		
		// Read lecture schedule.
		for (int i=0; i<n; i++) {
			int numT = stdin.nextInt();
			for (int j=0; j<numT; j++)
				lecs[stdin.nextInt()-1].add(i);
		}
		
		// Make a bipartite graph with lectures on one side, times on the other.
		Dinic g = new Dinic(n+MAXT);
		
		// Source to lectures.
		for (int i=0; i<n; i++) g.add(g.n-2,i,1,0);
		
		// Times to sink.
		for (int i=n; i<n+MAXT; i++) g.add(i, g.n-1, 1, 0);
		
		// Add in lectures iteratively to flow graph.
		int t, curres = 0;
		for (t=0; t<MAXT; t++) {
		
			// Add that these lectures occur at time t+1.
			for (Integer x : lecs[t])
				g.add(x, n+t, 1, 0);
				
			// Update flow for new edges.
			curres += g.flow();
		...
		// Due to how I am storing things.
...
class Dinic {
	...
	// Note - not clearing here...since I am rerunning it and want the flows to add.
	public int flow() {
